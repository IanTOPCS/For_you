...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(lcddev.id==0X9325||lcddev.id==0X4535||lcddev.id==0X4531||lcddev.id==0XB505||lcddev.id==0XC505)return r;	//這幾種IC直接返回顏色值
...
{					   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963)LCD_WR_REG(0X29);	//開啟顯示
	else if(lcddev.id==0X5510)LCD_WR_REG(0X2900);	//開啟顯示
	else LCD_WriteReg(0X07,0x0173); 				 	//開啟顯示
}	 
//LCD關閉顯示
void LCD_DisplayOff(void)
{	   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963)LCD_WR_REG(0X28);	//關閉顯示
	else if(lcddev.id==0X5510)LCD_WR_REG(0X2800);	//關閉顯示
	else LCD_WriteReg(0X07,0x0);//關閉顯示 
}   
//設置光標位置
//Xpos:橫坐標
//Ypos:縱坐標
...
//9320/9325/9328/4531/4535/1505/b505/5408/9341/5310/5510/1963等IC已經實際測試	   	   
void LCD_Scan_Dir(u8 dir)
{
	u16 regval=0;
	u16 dirreg=0;
	u16 temp;  
	if((lcddev.dir==1&&lcddev.id!=0X6804&&lcddev.id!=0X1963)||(lcddev.dir==0&&lcddev.id==0X1963))//橫屏時，對6804和1963不改變掃瞄方向！豎屏時1963改變方向
	{			   
		switch(dir)//方向轉換
		{
			case 0:dir=6;break;
	...
	if(lcddev.id==0x9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//9341/6804/5310/5510/1963,特殊處理
	{
		switch(dir)
		{
		...
		if(lcddev.id!=0X1963)//1963不做坐標處理
		{
			if(regval&0X20)
			{
...
{	
	LCD_WR_REG(0xBE);	//配置PWM輸出
	LCD_WR_DATA(0x05);	//1設置PWM頻率
	LCD_WR_DATA(pwm*2.55);//2設置PWM占空比
	LCD_WR_DATA(0x01);	//3設置C
	LCD_WR_DATA(0xFF);	//4設置D
	LCD_WR_DATA(0x00);	//5設置E
	LCD_WR_DATA(0x00);	//6設置F
}
...
void LCD_Display_Dir(u8 dir)
{
	if(dir==0)			//豎屏
	{
		...
		{
			lcddev.wramcmd=0X2C;	//設置寫入GRAM的指令 
			lcddev.setxcmd=0X2B;	//設置寫X坐標指令
			lcddev.setycmd=0X2A;	//設置寫Y坐標指令
			lcddev.width=480;		//設置寬度480
			lcddev.height=800;		//設置高度800  
		}else
		{
			lcddev.wramcmd=0X22;
	 		lcddev.setxcmd=0X20;
	...
	{	  				
		...
		{
			lcddev.wramcmd=0X2C;	//設置寫入GRAM的指令 
			lcddev.setxcmd=0X2A;	//設置寫X坐標指令
			lcddev.setycmd=0X2B;	//設置寫Y坐標指令
			lcddev.width=800;		//設置寬度800
			lcddev.height=480;		//設置高度480  
		}else
		{
			lcddev.wramcmd=0X22;
	 		lcddev.setxcmd=0X21;
...
{    
	...
	}else if(lcddev.id==0X1963)//1963豎屏特殊處理
	{
...
{ 	
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM4, ENABLE);  // 使能BANK1 
			...
			{ 
				...
				if(lcddev.id!=0X5310)		//也不是NT35310,嘗試看看是不是NT35510
				{
					...
					if(lcddev.id!=0X5510)			//也不是NT5510,嘗試看看是不是SSD1963
					{
						...
						if(lcddev.id==0X5761)lcddev.id=0X1963;//SSD1963讀回的ID是5761H,為方便區分,我們強制設置為1963
					}
				}
			}
 		}  	
	} 
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是這幾個IC,則設置WR時序為最快
	{
		//重新配置寫時序控制寄存器的時序   	 							    
		FSMC_Bank1E->BWTR[6]&=~(0XF<<0);//地址建立時間(ADDSET)清零 	 
		FSMC_Bank1E->BWTR[6]&=~(0XF<<8);//數據保存時間清零
		FSMC_Bank1E->BWTR[6]|=3<<0;		//地址建立時間(ADDSET)為3個HCLK =18ns  	 
		FSMC_Bank1E->BWTR[6]|=2<<8; 	//數據保存時間(DATAST)為6ns*3個HCLK=18ns
	}else if(lcddev.id==0X6804||lcddev.id==0XC505)	//6804/C505速度上不去,得降低
	{
		//重新配置寫時序控制寄存器的時序   	 							    
		FSMC_Bank1E->BWTR[6]&=~(0XF<<0);//地址建立時間(ADDSET)清零 	 
		FSMC_Bank1E->BWTR[6]&=~(0XF<<8);//數據保存時間清零
		FSMC_Bank1E->BWTR[6]|=10<<0;	//地址建立時間(ADDSET)為10個HCLK =60ns  	 
		FSMC_Bank1E->BWTR[6]|=12<<8; 	//數據保存時間(DATAST)為6ns*13個HCLK=78ns
	}
 	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID   
	...
	else if(lcddev.id==0x1505)//OK
	{
        ...
        LCD_WriteReg(0x0001,0x0100); 
        LCD_WriteReg(0x0002,0x0700); 
        LCD_WriteReg(0x0003,0x1038);//掃瞄方向 上->下  左->右 
        LCD_WriteReg(0x0004,0x0000); 
        LCD_WriteReg(0x000C,0x0000); 
        LCD_WriteReg(0x000F,0x0000); 
        LCD_WriteReg(0x0020,0x0000); 
	...
	}else if(lcddev.id==0X1963)
	{
		...
		LCD_WR_DATA(0x1D);		//參數1 
		LCD_WR_DATA(0x02);		//參數2 Divider M = 2, PLL = 300/(M+1) = 100MHz
		LCD_WR_DATA(0x04);		//參數3 Validate M and N values   
		delay_us(100);
		...
		LCD_WR_REG(0xE6);		//設置像素頻率,33Mhz
		LCD_WR_DATA(0x2F);
		LCD_WR_DATA(0xFF);
		LCD_WR_DATA(0xFF);
		
		LCD_WR_REG(0xB0);		//設置LCD模式
		LCD_WR_DATA(0x20);		//24位模式
		LCD_WR_DATA(0x00);		//TFT 模式 
	
		LCD_WR_DATA((SSD_HOR_RESOLUTION-1)>>8);//設置LCD水平像素
		LCD_WR_DATA(SSD_HOR_RESOLUTION-1);		 
		LCD_WR_DATA((SSD_VER_RESOLUTION-1)>>8);//設置LCD垂直像素
		...
		LCD_WR_REG(0xF0);	//設置SSD1963與CPU接口為16bit  
		LCD_WR_DATA(0x03);	//16-bit(565 format) data for 16bpp 

		LCD_WR_REG(0x29);	//開啟顯示
		//設置PWM輸出  背光通過占空比可調 
		LCD_WR_REG(0xD0);	//設置自動白平衡DBC
		LCD_WR_DATA(0x00);	//disable
	
		LCD_WR_REG(0xBE);	//配置PWM輸出
		LCD_WR_DATA(0x05);	//1設置PWM頻率
		LCD_WR_DATA(0xFE);	//2設置PWM占空比
		LCD_WR_DATA(0x01);	//3設置C
		LCD_WR_DATA(0x00);	//4設置D
		LCD_WR_DATA(0x00);	//5設置E 
		LCD_WR_DATA(0x00);	//6設置F 
		
		LCD_WR_REG(0xB8);	//設置GPIO配置
		LCD_WR_DATA(0x03);	//2個IO口設置成輸出
		LCD_WR_DATA(0x01);	//GPIO使用正常的IO功能 
		LCD_WR_REG(0xBA);
		LCD_WR_DATA(0X01);	//GPIO[1:0]=01,控制LCD方向
		
		LCD_SSD_BackLightSet(100);//背光設置為最亮
	}		 
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //開始寫入GRAM
		for(j=0;j<width;j++)LCD->LCD_RAM=color[i*width+j];//寫入數據 
	}		  
}  
//畫線
//x1,y1:起點坐標
